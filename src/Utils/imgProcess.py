...
class Process(Cali):
    def __init__(self,root,imgArr,accMode,Wscreen,Hscreen,chip="GJM02"):
    ...
    def main(self,root,accMode,Wscreen,Hscreen):
        ...
        sticker = cv2.bitwise_not(stickers)
        test = cv2.bitwise_and(processImg.copy(),processImg.copy(),mask=sticker)
    ...
    def findSticker(self,Proimg):
        ...
        mix = cv2.inRange(hsv.copy(),self.colDict['Black']["LL"],self.colDict['Black']["UL"])
        mix = cv2.morphologyEx(mix,cv2.MORPH_CLOSE,np.ones((7,7),np.uint8))
        mix = cv2.erode(mix,np.ones((7,7),np.uint8))
        blur = cv2.GaussianBlur(hsv,(3,3), 0)

        for colour,colRange in self.colDict.items():
            if colour == "Black" or colour == "Pin": continue
            thresh = cv2.inRange(blur,colRange["LL"],colRange["UL"])
    ...
    def colorSort(self,hsv,cnt):
        ...
        for col in self.colDict:
            ...
            if col == "Black" and hasCol[col] < 30000: hasCol[col] = 0
    ...
    def oddText(self,Bimg,cnt,M,a,type):
        ...
        if type in self.highlight.keys():
            cv2.putText(Bimg,str(a),(cX,cY),cv2.FONT_HERSHEY_COMPLEX, 1, self.highlight[type], 1)
    ...
    def normScan(self,Fimg,Bimg,preDef,oddDict,oddCol,Defects,length,cnt,pieces,Moments):
        ...
        for Col,val in oddCol.items():
            if Col in self.highlight.keys() and val > 0:
    ...
    def chipPixelCnt(self,root,Proimg,cnts,accMode,Wscreen,Hscreen):
        ...
        for cnt in cnts:
            ...
            if 100000<cArea or cArea<30: continue
            ...
            else: Fimg,Bimg,oddDict,Defects = self.normScan(Fimg,Bimg,preDef,oddDict,oddCol,Defects,length,cnt,pieces,Moments)
            print(oddCol)
