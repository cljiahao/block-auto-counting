...
def prep_cam(settings, light, mat, troubleshoot):
    ...
    if troubleshoot["Trouble"]:
        file_lists = os.listdir(dm.trouble_dir)
        ...
        file_path = os.path.join(dm.trouble_dir, file_name)
...
def save_img(img, mat, save_path):
    ...
    if rand_digit < 0.05:
        if not os.path.exists(dm.trouble_dir):
            os.makedirs(dm.trouble_dir)
        file_path = os.path.join(
            dm.trouble_dir, f"{mat}_{now.strftime('%d-%m-%y')}.png"
        )
        cv2.imwrite(file_path, img)
